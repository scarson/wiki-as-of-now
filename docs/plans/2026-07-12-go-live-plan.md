...
**Overall:** ALL 7 PHASES SHIPPED 2026-07-12. Production live at https://wikinow.scarson.io; cron registered (first tick 2026-07-13T00:00Z, watch scheduled). Open Sam-side items: OAuth redirect-URI registration; first OAuth sign-in exercises the enqueue toast.
...
| 6 — Cron enable (production, LAST) + first-tick watch | ✅ Shipped + FIRST TICK VERIFIED | merge `b97ca71` | the cron PR [#30](https://github.com/scarson/wiki-as-of-now/pull/30) merged on green CI; production research worker redeployed with `schedule: 0 */8 * * *` (version `39df054a`, ~18:45Z — cron registered AFTER the 16:00Z boundary, so the FIRST tick is 2026-07-13T00:00Z; in-session watch scheduled at 00:13Z). Interval derivation in the config comment + PR body |
| 7 — Promote dev→main, cleanup, report | ✅ Shipped | merge `492aa35` | release PR [#31](https://github.com/scarson/wiki-as-of-now/pull/31) merged; main-push Deploy run 29204626964 green with guarded steps EXECUTED against production (migrations no-op, both workers redeployed, cron intact — production CD proven); credential files ×3 + exclude block deleted; ai-probe-scratch worker deleted; research-dlq-dev purge SKIPPED (permission classifier — contents inert, Sam may purge); memory updated |
...
- **workerd's global `fetch` validates its receiver — two more fixture-blind live defects (defects #4/#5 of the smoke), same class as the Gemma trio:** (1) `BraveSearchProvider`'s default `fetchFn = fetch` (a detached global) invoked as `this.fetchFn(...)` threw `TypeError: Illegal invocation` before any network I/O → every search failed as `provider_unavailable` → infinite retry loop (the real cause of the handoff's "one open problem"; root-caused via new codes-only search-seam telemetry `research.search.failed{status,reason}`, fixed at `96d8cd8` by defaulting to a lambda). (2) The research worker passed `fetchImpl: fetch` into `fetchSourceText`, which called it as `opts.fetchImpl(...)` → same TypeError → every source fetch silently returned `network_error` → triage saw zero pages → four false `no_proposals` packs with EMPTY dispositions (deleted from the dev data table per the existing defect-artifact precedent; audit log untouched; fixed at `d074f42` with a receiver-neutral call site + caller lambda). Node's fetch is receiver-insensitive, so no unit test could see either defect — both receiver contracts are now pinned by tests. Rule: NEVER pass a detached global `fetch`; wrap in a lambda (`(u, i) => fetch(u, i)`). The bare-call sites in `wikimedia.ts`/`pageviews.ts` (receiver `undefined`) are safe and proven live.
...
**Execution Status:** ✅ Tasks 3.1–3.5 DONE as of 2026-07-12 ~08:25Z (continuation session; Task 3.6 PR next). Dev live at wiki-as-of-now-dev.samuel-carson.workers.dev. **Task 3.5 smoke PASSED:** after Sam authorized the secret puts (classifier had blocked them; ADMIN_SECRET re-put), the codes-only telemetry named the real failure in two hops — `research.search.failed{transport, TypeError}` — root-caused to TWO workerd detached-global-`fetch` receiver defects (see Discoveries; pitfalls AI-2; fixed `96d8cd8` + `d074f42`, receiver contracts pinned by tests). Four defect-artifact `no_proposals` packs deleted from the dev data table (audit log untouched, per precedent), re-enqueued, and the pipeline then produced REAL packs: Three Gorges Dam + California High-Speed Rail both `proposals_present` with 1 card each, `model_version @cf/google/gemma-4-26b-a4b-it`, real verbatim-verified quotes + resolving URLs (IBTimes card verified in-browser), audit rows codes-only, stub count 0. Task 3.4 QA COMPLETE: nav/queue/anonymous-401/worksheet PASS at desktop + mobile in dark (light theme N/A by design — single dark archival palette, DESIGN.md §2); post-pack surfaces verified live: supported honesty banner, evidence card, source-open gate confirm→unlock→"source opened and logged", snippet assembler + mechanical citation + G12 disclosure summary, transparency page (selected/dropped/4 neutral queries + model line), About page incl. the new Wikipedia-guideline link (`77ff557`, Sam's mid-session request). ONE exception: the enqueue-success toast ("Queued N for research…") was NOT exercised live — it needs an authed browser session and dev's SESSION_SECRET value died with the prior session (rotation not authorized); the API path returns 202 (proven via admin-header enqueues) and the copy is code-pinned at `src/app/queue/page.tsx:121`. Deferred to Sam's first OAuth sign-in on production.
...
**Execution Status:** ✅ SHIPPED — merge `b97ca71` (PR #30); production research worker redeployed with the cron (version `39df054a`). Interval `0 */8 * * *` chosen over `0 */6 * * *` — the theoretical all-timeout worst (~5.8h) leaves no margin under 6h, and 8h costs zero yield (the 10-pack/day quota is consumed by the first tick either way). FIRST TICK 2026-07-13T00:00Z (registration landed ~18:45Z, after the 16:00Z boundary); Step 4 verification scheduled in-session at 00:13Z — if the session is gone, run the Step 4 checks by hand against the `research` queue + prod D1. **FIRST TICK VERIFIED at 00:13-00:20Z:** 10 packs committed (exactly the 10/day quota cap), 4 with evidence cards, all `model_version @cf/google/gemma-4-26b-a4b-it` (no stub); audit tally research.completed×10 + research.quota_exceeded×4 (expected ACKed drops) + research.unavailable×3 transient (max 2 failed attempts on any one claim — below the 4-attempt DLQ threshold, so the DLQ is provably empty; both claims recovered on retry); queue drained and idle (90s tail captured zero events); production worksheets 2/4/5 render cron-produced evidence cards + transparency links with the supported banner, worksheet 3 shows the honest no-strong-source state.
...
**Execution Status:** ✅ SHIPPED — release PR #31 merged at `492aa35` 2026-07-12 ~18:50Z; production CD proven (run 29204626964: guarded steps EXECUTED — migrations no-op against prod D1, both workers deployed, cron preserved). Cleanup: credential files ×3 deleted + `.git/info/exclude` block removed (main checkout clean); `ai-probe-scratch` worker deleted; scratchpad secret files removed; `research-dlq-dev` purge skipped (permission classifier denied — the ~8 dead messages are inert; purge by hand if desired). Memory: `v1-build-status` finalized, `workerd-fetch-receiver` + `ask-in-plain-text` added. Journal: private-journal MCP absent from the continuation session — reflections captured in pitfalls AI-2 + memory instead. Final report delivered in-session.
...
- [ ] **Step 2 (cleanup):** `rm brave_api_key.txt google_oauth_clientid.txt google_oauth_client_secret.txt` (repo root) + scratchpad secret files; remove their `.git/info/exclude` block; verify `git status` clean. ALSO: delete the diagnostic worker `bunx wrangler delete --name ai-probe-scratch` (a scratchpad probe deployed 2026-07-12 during the Gemma debugging; it carries a BRAVE_API_KEY secret that dies with it); optionally purge the ~4 dead messages parked in `research-dlq-dev` from the pre-fix retry storms (`bunx wrangler queues purge research-dlq-dev` — destructive but the DLQ contents are superseded; or leave them, they are inert).
