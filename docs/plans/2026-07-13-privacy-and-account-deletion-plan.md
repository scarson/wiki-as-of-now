...
- [ ] **Step 1: No third-party analytics/tracking/ad cookies or scripts.** Grep `src/` for analytics/gtag/plausible/posthog/segment/ga/fbq/doubleclick; inspect `layout.tsx`/`globals.css`. Expected: none (only session + OAuth cookies).
- [ ] **Step 2: Anonymous use associates nothing with a person.** Confirm anonymous branches resolve to a role label (post-Phase-7: `"AnonUser"`), never a client-supplied identifier. Record findings for the PR body.
...
- [ ] **Step 1: Write the policy** — copy the **finalized text verbatim from design §3.2** ("Proposed policy text" block; it is stop-slop-cleaned and Sam-approved). Keep `_Last updated: 2026-07-13_` and the `[GitHub Discussions](…/discussions)` markdown link.
- [ ] **Step 2: Commit.** `git add docs/policy/privacy-policy.md && git commit -m "docs(policy): add privacy policy"`
...
- [ ] **Step 1: Add dep (verified provenance).** `pnpm add markdown-to-jsx`; verify `node_modules/markdown-to-jsx/package.json` `repository` → quantizor repo; pin the exact resolved version; commit lockfile.
- [ ] **Step 2: Create the page** (`src/app/privacy/page.tsx`):
```tsx
...
```
- [ ] **Step 3: Link `/privacy`** from the home footer (`page.tsx`) and About footer (`about/page.tsx`) — plain `next/link` next to "About & compliance".
- [ ] **Step 4: Verify.** `tsc`, `eslint`, `npm run build` — confirm `/privacy` is `○ (Static)` and the built HTML contains the policy headings. If read fails, take the fallback.
- [ ] **Step 5: Commit.** `feat(web): render privacy policy at /privacy`.
...
- [ ] **Step 1: Failing test** (`test/workers/account-delete-route.test.ts`):
```ts
...
```
- [ ] **Step 2: Run → FAIL** (route missing): `node_modules/.bin/vitest run -c vitest.workers.config.mts test/workers/account-delete-route.test.ts`
- [ ] **Step 3: Endpoint** (`src/app/api/account/delete/route.ts`):
```ts
...
```
- [ ] **Step 4: Run → PASS.** Then `tsc`.
- [ ] **Step 5: Commit.** `feat(account): POST /api/account/delete (null-attribution + profile delete)`.
...
- [ ] **Step 1: Extend the authenticated branch.** Replace the shared `busy` with `const [op, setOp] = useState<"idle" | "signout" | "delete">("idle")`; keep a `deleteFailed` that resets whenever the confirm opens/cancels. Sign-out uses `op === "signout"`; delete uses `op === "delete"`. Add a `deleteAccount()` handler POSTing `/api/account/delete` → on ok `setAnonymous()` + `router.refresh()`, else set failure. In the authed row, after Sign out:
```tsx
{!confirming ? (
...
- [ ] **Step 2: Verify.** `tsc` + `eslint`. Post-deploy browser QA (needs a session): Delete → confirm → account gone, signed out, nav shows "Sign in"; Cancel dismisses; a re-login makes a fresh account; while deleting, Sign out/Cancel are disabled.
- [ ] **Step 3: Commit.** `feat(ui): account menu with delete-account confirmation`.
...
- [ ] **Step 1.** In both routes, change `actor = auth.kind === "authenticated" ? auth.userId : "system"` → `: "AnonUser"`.
- [ ] **Step 2.** Update the test asserting `actor === "system"` for an anonymous source-open ([sources-open-route.test.ts:67](../../test/workers/sources-open-route.test.ts)) → `"AnonUser"`. Grep for any other test asserting `"system"` on these routes' anonymous path.
- [ ] **Step 3: Verify.** `node_modules/.bin/vitest run -c vitest.workers.config.mts` green; `tsc`.
- [ ] **Step 4: Commit.** `fix(audit): label anonymous actions AnonUser, not system`.

---

## Finalization

- [ ] Full verification: both pools green, `tsc`+`eslint` clean, `npm run build` OK (`/privacy` static).
