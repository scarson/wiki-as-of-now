...
- **[PR #47](https://github.com/scarson/wiki-as-of-now/pull/47) — anonymous capture throttle.** In-memory per-IP fixed window (10 req/60 s, env-overridable) on `POST /api/queue/capture`; 429 + Retry-After; cross-origin POSTs refused before charging the budget (codex P2). **The load-bearing constraint:** the privacy policy's "anonymous actions carry no personal identifier" promise rules out any durable (even hashed) IP storage without a Sam-approved policy amendment. **⚠ Superseded same day by [PR #52](https://github.com/scarson/wiki-as-of-now/pull/52):** Sam challenged the rationale for rejecting the Workers Rate Limiting binding (both stated reasons were wrong); the in-memory limiter was replaced by the `CAPTURE_RATE_LIMITER` `ratelimits` binding and deleted. Current design + correction log: [capture-throttle design](../design/2026-07-18-capture-throttle-design.md).
