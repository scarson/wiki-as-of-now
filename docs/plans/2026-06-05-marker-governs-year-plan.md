...
- **Task 2.4 — named-entity discriminator: the plan's starter correctly identifies the core pattern but required two precision guards beyond the plan's spec.** (1) **Temporal-preposition-as-token guard:** the regex `/\b[A-Z][A-Za-z0-9.&-]+/` matches sentence-initial prepositions like "After", "From", "Before" (capitalized). Without a guard, "After 2020, the Army planned to buy" drops 2020 (over-drop) because "After" looks like a proper noun. Added `TEMPORAL_PREP_WORD` check: if the token itself is a temporal preposition (case-insensitive), it is a frame, not an entity — keep the year. Also added "from" to this list (handles "From 2015 to 2022, … will be manufactured"). (2) **Marker-position guard:** corpus scan revealed the gold positive "The Navy will request FY 2022 funding" was being dropped — "FY" matches as a capitalized abbreviation, but the marker "will" precedes 2022 in its own clause, making 2022 the marker's complement target. Added: if `markerIndex >= 0 && markerIndex < occ.start` (marker before year), keep the year. This is the same structural insight as Task 2.3's key discriminator. (3) **Gold-set correction:** `test/gold/gold-set.json` had an incorrect `stale: true` entry for `panzerhaubitze_2000.wikitext` with `expectedYear: 2000`, where 2000 is the product name "PzH 2000" — not a stale claim year. The actual sentence says "expected to be ready by 2028" (2028 is future, not stale). The det3-fp-set correctly labels the same sentence `stale: false, subShape: named-entity`. Corrected the gold-set entry to `stale: false` with a note explaining the labeling error. This reduces the gold positive count from 33 to 32 (TP goes 33→32, precision stays at 0.9697 = 32/33). All 3 curated named-entity FPs drop (0 residual); corpus flag-diff = exactly 3 drops (CES 2025, PzH 2000, MSPO 2024), **0 newly-flagged sentences**; reachable recall 1.0 (no give-back). `named-entity` hard-gated to `expect([])` in det3-fp.test.ts.
- **Task 2.3 — deadline-frame escape (review SHOULD-FIX, `1cbe407`).** The marker-position noun-modifier rule over-dropped a *leading* deadline frame when the marker trailed the year ("By the 2024 election, the bill will pass" dropped 2024, a real target). Added a deadline-preposition escape (`by|before|until the <year> <event>` → KEEP regardless of marker position; `during|in|after` remain background → DROP). Recovers the over-drop; verified it un-drops none of the 9 curated noun-modifier FPs; precision 0.9706/recall 1.0 held.

- **Task 2.5 — parenthetical/range discriminator: the plan's starter required two corrections and one extension beyond its spec.** (1) **`from...to` window too narrow for the end year:** the plan's starter looked only 10 chars before the end year, so the `from` keyword (up to ~15+ chars back) was invisible; 2003 in "from 1995 to 2003" was not dropped. Fixed by scanning the full sentence with a regex (not a window), matching each occurrence's character offset against the match span. (2) **Sentence-initial `From X to Y` must be KEPT:** the design §5 / README "excluded as not-incidental" explicitly lists "From 2015 to 2022, 24 units will be manufactured" (k9_thunder) as a range that is the marker's own temporal window; the plan's starter used `^[^(]*\)` which did not guard against this. Fixed by adding a `sentenceInitialSafe` flag: when the `from...to` match starts at position 0, the year is KEPT. (3) **`between <year> and <year>` extension:** the plan's spec listed only `from...to` and `<year>[–-]<year>` for ranges, but the real corpus FP for high_speed_2 (anchorYear 1966) is in a "between 1838 and 1966" range. The year regex only covers 1900–2099, so 1838 is invisible — 1966 is the sole detectable year and needed `between...and` support to be dropped. Added `between...and` as a third range pattern (no sentence-initial exception). Also removed the now-unused `flaggedOnAnchorYear` helper from det3-fp.test.ts (lint error: defined but never used after the baseline-reporting block was replaced with hard gates). All 6 parenthetical + 2 range curated FPs dropped (0 residual); corpus flag-diff = 15 fewer flags, 1 newly-flagged sentence (high_speed_2 "… was expected to have insufficient capacity around 2025" — this is the README mixed case, previously masked by incidental 1995/2015 anchor; a genuine stale claim correctly surfaced); precision 0.9697 (32 TP / 1 FP, no new gold FP dropped); recall floor 1.0 (no give-back).

### Discoveries

- **A pre-existing precision-gold MISLABEL was surfaced by the lever (Task 2.4) and corrected.** `gold-set.json`'s `panzerhaubitze_2000` entry was labeled `stale:true / expectedYear:2000` with a note claiming "expected to be ready by 2000" — but the actual fixture sentence (line 96) says "expected to be ready **by 2028**" (future, not stale); 2000 is the product name "PzH 2000 A5". The original labeler mirrored the OLD detector's wrong anchor (it anchored to the product-name year via `Math.min`). Independently verified (controller + reviewer both read the sentence). Same class as the recall-work's fehmarn cross-sentence-resolution fix — the gold set has had >1 mislabel that only surfaces when a detector change touches the relevant sentence. Phase 3 / methodology should note this and that the gold set is built from (occasionally-wrong) detector output, so detector changes are a useful audit of the gold.
- **`isNamedEntity` has two known over-KEEP residuals (Task 2.4 review, accepted-not-fixed).** A genuine entity year is wrongly KEPT (a residual FP, not a regression) when (a) the entity sits *after* the marker ("models will be shown at CES 2025" — the marker-position keep-guard fires) or (b) the entity follows a *bare* temporal prep ("During CES 2025 …" — the bare-frame keep-guard fires). Neither occurs in the 136-fixture corpus or gold set, and tightening (a) risks the load-bearing "FY 2022" recall positive that the marker-position guard rescues — so accepted per the precision-over-recall stance + Sam's "tiny recall give-back" guidance. Document in the methodology as a known named-entity residual; candidates for a future precision-tightening pass.
