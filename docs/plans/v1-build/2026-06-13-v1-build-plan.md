...
| 7 — Provision & deploy prep | ✅ Shipped | `bb1f14d`…`e0119fb` | on `feat/v1-build`; 895 node + 26 workerd green; report `build-reports/phase-7.md`; Review trigger (deploy topology + CI gates); Sam-gated go-live steps remain (provision/deploy/secret-put/cron via `docs/runbooks/go-live.md`) |
...
- **Phase 7 / D-1 — AI-binding config test asserts the real shipped shape, not the plan's idealized `{ binding: "AI" }`.** Phase 1 deviation D4 added `remote: true` to both AI bindings, so the plan's Task 7.1 `toEqual({ binding: "AI" })` would fail against the config that ships. Per testing-pitfalls §6 (test the real shipped config), the test asserts `cfg.ai?.binding === "AI"` — pinning the binding without freezing the `remote` flag. No config edit was needed (the bindings were already correct; Task 7.1 was verify-only). See `build-reports/phase-7.md` D-1.
- **Phase 7 / D-2 — the deploy-pipeline dormancy guard is a STEP-level `if:`, not the plan's JOB-level `if:`.** GitHub Actions forbids the `secrets` context in a job-level `if:` (only `github`/`needs`/`vars`/`inputs` are available there — verified against the GitHub Actions contexts docs), so the plan sketch's `jobs.deploy.if: ${{ secrets.CLOUDFLARE_API_TOKEN != '' }}` would silently never run. Corrected to a step-level `if: ${{ secrets.CLOUDFLARE_API_TOKEN != '' }}` on each deploy/migrate step (the secrets context IS available at step level), with the token mapped to job `env:`; steps skip cleanly (green, not red) when the secret is absent. The Task 7.6 test asserts the step-level form. See `build-reports/phase-7.md` D-2.
- **Phase 7 / D-3 — deploy-pipeline branch→env test asserts the real DRY `--env ${{ steps.env.outputs.name }}` mapping, not literal `--env production`.** The pipeline resolves the branch to an env once (`name=production` for `main`, else `name=dev`) and feeds every deploy the templated env, so the literal `--env production`/`--env dev` strings never appear. The Task 7.6 test asserts the resolve mechanism rather than denormalizing the pipeline into duplicated literal-env branches — same reconciliation class the plan blesses for the Task 7.5 CI-invocation regex. See `build-reports/phase-7.md` D-3.
- **Phase 7 / D-4 — provisioning doc + runbook use the real Phase-5 secret names (`ADMIN_SECRET`, not the sketch's `ADMIN_FLAG`).** Shipped Phase-5 code uses `ADMIN_SECRET` (an `x-admin-secret` header — Phase 5 D-3) plus `SESSION_SECRET`; `scripts/provision.md` and the runbook document the real set (`SESSION_SECRET`/`ADMIN_SECRET`/`GOOGLE_CLIENT_ID`/`GOOGLE_CLIENT_SECRET` on the app worker; `BRAVE_API_KEY`/`RESEARCH_KILL_SWITCH` on the research worker), and the drift guard pins them. See `build-reports/phase-7.md` D-4.
...
- **Audit-trail readers built-and-tested-but-unwired, pending a future admin/transparency surface.** `readAuditTrail` and `summarizeFeedback` (`src/db/audit-queries.ts`) are consumed only by tests. They are deliberately *global*, *admin-scoped* reads: `readAuditTrail` returns the entire `audit_log` (all actors, all event types, with per-row JSON isolation so one corrupt row degrades rather than aborting — CC-19), and `summarizeFeedback` counts `session.feedback` rows across all claims. The existing per-claim transparency page (`src/app/articles/[id]/transparency/page.tsx`) is *pack-scoped* — it shows how a single claim was researched via `getSurfaceablePack`/`toTransparencyView` — so wiring these whole-log readers into it would be a scope mismatch (it would surface unrelated audit rows and other actors on a single-claim view). They are intentionally retained as tested infrastructure for a near-future admin/transparency surface (e.g., an aggregate "what the system has done" dashboard or a feedback-outcome summary). Do NOT remove them; do NOT wire them into the current pack-scoped transparency page. (integration-review FIX 6.)
...
**Execution Status:** ✅ SHIPPED on 2026-06-13 (branch `feat/v1-build`, commits `bb1f14d`…`e0119fb`, 7 commits). All 8 tasks done; final suite green (tsc + lint clean, 895 Node + 26 workerd; research `--dry-run` bundles credential-free). Build report: [build-reports/phase-7.md](build-reports/phase-7.md). **Sam-gated steps remain** (provision, deploy, secret-put, cron-enable — all documented in `docs/runbooks/go-live.md`, none executed). **Merge: Review — architecture (deploy topology + CI gates); Sam merges (agent does NOT self-merge).** Deviations D-1..D-4 (Phase 7) in the top-of-plan Deviations subsection.
