...
**Overall:** In progress. 2/2 phases built; Phase 2 in PR review.
...
### Phase 1 batch review (3 rounds — reports in `docs/plans/phase1-review/`)
Per the after-Phase-1 block: correctness, compliance-guardrail adherence, and test rigor, each dispatched to a subagent that persisted findings to a file (ORCH-1). Outcome → all actioned in `1d592cd`:
- **Correctness:** 1 IMPORTANT — `migrations_dir` was at the wrong level in `wrangler.jsonc` (top-level, silently ignored; belongs in each `d1_databases[]` entry). Fixed + verified. Plus deferred findings below.
- **Compliance:** COMPLIANT — no guardrail violations. (One MINOR finding, "audit log has no test coverage," was a **factual error** by the reviewer — `test/db/audit-log.test.ts` exists and tests append/read + the append-only invariant; its valid kernel, "assert the payload is identifiers-only," overlapped a test-rigor finding and is now covered.)
- **Test rigor:** 2 IMPORTANT (untested `stale_candidates`/`articles` schema; untested provider-rejection/retry semantics) + minors — all fixed; suite grew 8→12 tests, deterministic across runs.

### Task completion log — Phase 2
All via implementer subagent (strict TDD) → spec/code review subagent → controller triage/fix/commit. Gates per task: `pnpm test` green, `pnpm exec tsc --noEmit` clean, `pnpm lint` clean.
- **Task 2.1** (domain types): ✅ DONE — `fec5bd3`. Review ✅. Controller strengthened the verbatim test to construct `ParsedArticle` (the verbatim import was otherwise unused → lint error).
- **Task 2.2** (article parser): ✅ DONE — `6e7b198`. Review ✅ (no must/should-fix). `wtf_wikipedia` ships its own TS types; no `.d.ts` needed. wtf `depth()` returns 0 for both lead and top-level sections (lead distinguished by `heading === ""`).
- **Task 2.3** (markers + years): ✅ DONE — `fea0adb`. Review ✅. `MARKER_STRENGTH` is the single source; word-boundary + case-insensitive matching; years 1900–2099 in order.
- **Task 2.4** (suppression): ✅ DONE — `1089fe0`, refined in `fb14c94`/`cb4be1d` (see Discoveries). Review ✅ (grouped year regex confirmed). Veto-strength penalty (100) so a fired rule out-weighs `temporalRisk`.
- **Task 2.5** (scoring): ✅ DONE — `563324a`. Review ✅ (no must/should-fix). Deterministic template explanation; 0 when `year >= asOfYear`; unknown marker → 0 not NaN.
- **Task 2.6** (detect orchestration): ✅ DONE — `32fe8b1`. Review ✅ (no must/should-fix; **compliance lens: provably LLM-free**). Controller added a `DETECTOR_VERSION` assertion (verbatim import was otherwise unused → lint). `DETECTOR_VERSION = "1.0.0"`.
- **Task 2.7** (fixtures + gold + precision gate): ✅ DONE — `fb14c94` (suppression tuning) + `ac3255b` (fixtures/gold/gate), gold updated `3c20d3d`. Gold-set precision **1.0** (gate ≥0.9). Gold set built empirically from real detector output on 4 real fixtures; 5 positives / 6 negatives + a composition guard so the gate can't be gamed by deleting negatives.

### Phase 2 batch review (3 rounds — reports in `docs/plans/phase2-review/`)
Per the after-Phase-2 block: correctness, purity/no-LLM compliance, gold-set honesty — each persisted to a file before returning (ORCH-1); review subagents inspected read-only and did not move HEAD (ORCH-2).
- **Round 1 — Correctness:** 3 over-broad-suppression findings actioned in `cb4be1d` (dateline month slot matched the filler "the"; Rule 3 fired on bare "later"; incoherent Rule 1/detect comments). Remaining findings are accepted precision-over-recall recall gaps, now documented (Discoveries + DET-2). Verified-correct: markers word-boundary, year bounds, score arithmetic, parse robustness, tie-breaking, sort stability.
- **Round 2 — Compliance:** **COMPLIANT.** Detector + full import graph have zero model/network/clock/async calls and no `src/research/` import; explanations are template-filled. Verified by grep + a `Date`-monkey-patch probe + a 5-run determinism check; `wtf.fetch` is never on the `wtf(string)` path.
- **Round 3 — Gold-set honesty:** Precision gate **HONEST**, not gamed. Gold set has real negatives (composition-guarded); labels confirmed by independent re-derivation; the suppression rule generalizes across 14 novel probe sentences. Actioned: added the higher-scoring Zumwalt FY2022 positive (`3c20d3d`), corrected the SBX-1 negative note, flagged the min-year dependency.

### Deviations
- **Phase 2 toolchain — pnpm pinned to 11.5.1; build allowlist moved to `pnpm-workspace.yaml` (`bd6ca3a`).** The container provided pnpm 11 + Node 22 (Phase 1 ran on pnpm 10.33 + Node 24). pnpm 11 ignores `package.json`'s `pnpm.onlyBuiltDependencies`, so native `better-sqlite3` was left uncompiled and the DB tests could not load it. Pinned pnpm via the corepack-standard `packageManager` field (latest stable, at Sam's request) and migrated the build allowlist to `pnpm-workspace.yaml`'s `allowBuilds` map. ⚠️ **`.nvmrc` still pins Node 24, but only Node 20/21/22 are available in this environment — Phase 2 ran on Node 22.** Flagged to Sam; not changed.
- **Phase 2 verbatim tests — small strengthenings for lint (`fec5bd3`, `32fe8b1`).** The plan's verbatim tests for Task 2.1 and 2.6 import a symbol they don't assert on (`ParsedArticle`, `DETECTOR_VERSION`), which trips the no-unused-vars lint gate. Resolved by *adding* an assertion that exercises the symbol (constructing a `ParsedArticle`; asserting `DETECTOR_VERSION === "1.0.0"`) — a strengthening, not a weakening. The Task 2.7 precision test's `as any[]` was replaced with a typed `GoldEntry[]` interface (no-explicit-any gate).
- **Task 2.4 suppression refined during Task 2.7 + review (`fb14c94`, `cb4be1d`).** Sanctioned by Task 2.7 Step 3 ("tune … by improving suppression"). Rule 1 was rewritten from "leading bare-`In YYYY` frame + a narrow past-tense verb cue" to "leading temporal frame (In/By/During/As of + optional month/qualifier + year) **whose year equals the claim's anchor year**" — giving the previously-reserved `year` parameter a real use and catching the dominant real false-positive class (see Discoveries / DET-1). Added an `i` flag to the frame for case consistency. The month slot was later constrained to real month names + early/late/mid (it had matched the filler "the"), and Rule 3 was tightened to require a resolution verb. All existing 2.4 tests still pass; new tests lock the refinements.
...
### Discoveries
- **Phase 2 corpus expansion (round 2) — 100 fixtures across new domains; mid-sentence attribution (Rule 4) + a new irreducible FP class (DET-3).** Added 50 more fixtures from space/launch, rail & transit, infrastructure/dams, nuclear/energy, civil aviation, and naval (different prose styles, heavier use of `is scheduled to`/`to be completed by`). Implemented `suppress.ts` **Rule 4** (mid-sentence attribution: a past reporting/event verb followed by an `on/in <date>` of the claim year — "X announced plans to … in January 2010", "Reuters reported on 1 June 2022 that …"); verified it suppresses the clear cases without flipping any of the 21 gold positives. Gold set → 59 entries (precision 1.0). The new domains surfaced **DET-3 — incidental historical years**: a forward "will/marker" statement carrying an unrelated past year (founding/launch dates, year ranges, "HMS Belfast (1938)") flags at that old year with a high score. No safe deterministic fix exists (range/verb rules over-suppress real positives or break forward claims), so it is documented as an accepted FP floor, not patched. (`src/detector/suppress.ts`, `docs/pitfalls` DET-2/DET-3.)
- **Phase 2 corpus expansion — fixture corpus grown ~10× (4 → 50 real articles); leading "On <full date>" datelines are the dominant FP at scale.** Sam requested an order-of-magnitude larger corpus. Added 46 obscure equipment/procurement articles (artillery & munitions, sensors/radar, drones/UAS, ground vehicles, experimental programs). Running the detector across them showed the largest remaining FP class is leading full-date narration ("On 30 August 2018, the Navy announced…"), which the In/By/During/As-of-only dateline rule missed. Generalized `DATELINE_REGEX` to `In|By|During|As of|On` + up to three date-ish tokens (day-number | month | early/late/mid) + year (commit `a372b3c`); suppressed 12 more corpus FPs with no loss of genuine positives. Gold set expanded to 39 entries (21 positives / 18 negatives) across 15 fixtures, precision still 1.0. The remaining residual FP is **mid-sentence attribution** ("X reported on <date> that … plans to …"), left unlabeled and documented (DET-2). Review: `docs/plans/phase2-review/round-4-corpus.md`.
- **Phase 2 — Historical dateline narration is the dominant detector false-positive class (now `docs/pitfalls` DET-1).** On real procurement articles the majority of raw "marker + past year" flags are narration of a past announcement/statement ("In March 2013, X announced plans to…", "In 2008, X said … will…", "In 2021, … no plans to…"). The naive marker+year detector had poor precision until Rule 1 was rewritten to suppress leading-dateline-whose-year-is-the-claim-year. Building the gold set from *real detector output* (not idealized sentences) is what surfaced this. (`src/detector/suppress.ts` Rule 1.)
- **Phase 2 — Accepted precision-over-recall recall gaps (now `docs/pitfalls` DET-2).** Deliberate, documented, all recall-only (never precision) losses: (a) **inline-year requirement** — a claim with no 4-digit year in the same sentence is never flagged (e.g. SBX-1's famous "scheduled to be based in Adak Island" has no inline year, so it is missed); (b) **earliest-past-year + dateline** — `detect.ts` anchors to the earliest past year, so "In 2015, … expected to deliver in 2020." is suppressed on the 2015 dateline and the 2020 target is missed (preferring the later year would re-flag the ambiguous historical-announcement negatives and *lower* precision — the earliest-year choice is the precision-favoring one); (c) **`By <year>` deadline** — "By 2025, … will …" is suppressed by the same rule that correctly kills "By May 2022, the Navy shifted its plans…"; (d) **mid-sentence attribution residual FP** — "A report … on a January 2012 wargame … stated that … plans to…" still flags (the dateline rule is leading-frame only); this is the one remaining unlabeled false positive in the Littoral fixture. Fixing (d) needs a separate mid-sentence-attribution rule (future precision work).
- **Phase 2 — `wtf_wikipedia` `depth()` is 0 for both the lead and top-level (`==`) sections.** `Section.level` is wtf's `depth()` (0-based: lead=0, `==`=0, `===`=1). The lead is distinguished only by `heading === ""`. `level` is stored but not currently used by the detector; consumers must not rely on `level` to find the lead.
- **Phase 2 — `extractYears` extracts both endpoints of a hyphenated range** (`"2006-2007"` → `[2006, 2007]`, the hyphen is a word boundary). Combined with the earliest-year selection this can anchor a candidate to a range's start year. Acceptable; noted for awareness.
...
- **FK enforcement differs local vs D1.** better-sqlite3 leaves `foreign_keys` OFF by default; D1 enforces them. Without `PRAGMA foreign_keys = ON`, local tests can false-pass on referential-integrity violations (e.g. `stale_candidates.page_id → articles`). Fixed in `openLocalDb` (`c1cdd9a`) and in `test/helpers/db.ts:freshTestDb()`; a regression test now asserts the FK fires (`1d592cd`). Belongs in `docs/pitfalls/testing-pitfalls.md`.
- **✅ RESOLVED — `articles.page_id` NULL-safety (`WITHOUT ROWID`).** Test-rigor review flagged that `articles.page_id INTEGER PRIMARY KEY` silently auto-assigns a rowid on a `NULL` insert, yielding an article row with no real Wikipedia page ID. **Verified finding (SQLite 3.53.1): `NOT NULL`, `NOT NULL PRIMARY KEY` ordering, and `CHECK (page_id IS NOT NULL)` are ALL no-ops** on a rowid-alias PK — NULL is replaced by an auto-rowid *before* those constraints evaluate. The effective fix (Sam-approved) is to make `articles` a `WITHOUT ROWID` table with `page_id INTEGER PRIMARY KEY NOT NULL`, which rejects NULL and is the more correct schema (page_id is a true natural key). `WITHOUT ROWID` cannot be applied via `ALTER`, so this edited the never-deployed `0001_init.sql` (+ byte-identical `schema.sql`) directly — safe because no migration has run against any real D1 (placeholder `database_id`). Regression test asserts NULL is rejected and the `stale_candidates → articles` FK still fires. Captured in `docs/pitfalls/` as a SQLite gotcha.
- **DEFERRED (observation) — `AuditEntry.payload`/`makeAuditLog.append` do not guard `JSON.stringify`.** `payload: undefined` → `NOT NULL` DB error; a circular object → throw. Callers control payload (plain identifier objects) so there's no live bug; identifiers-only is enforced by convention + documented in three places, not by the type. Not worth a validation guard at skeleton stage (YAGNI); revisit if/when an event taxonomy is introduced.
