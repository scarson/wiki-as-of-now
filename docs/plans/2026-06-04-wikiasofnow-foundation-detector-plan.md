...
- **⚠️ DEFERRED for Sam — `articles.page_id INTEGER PRIMARY KEY` permits a NULL insert (SQLite auto-assigns a rowid).** Test-rigor review flagged that a `NULL` page_id would silently get a fabricated rowid rather than being rejected, yielding an article row with no real Wikipedia page ID. The app always inserts a real `page_id`, so there is no live bug, and the migration SQL is the plan's verbatim foundational schema — I did **not** change it on a drive-by. Recommended hardening when the data layer is wired: add an explicit `NOT NULL` (verify the exact `INTEGER PRIMARY KEY NOT NULL` semantics in SQLite/D1 first, since `INTEGER PRIMARY KEY` is a rowid alias) — and do it as a NEW additive migration, not by editing `0001_init.sql`, if `0001` has shipped to any real D1 by then.
