...
- **The deterministic guard is unusable** — 0.125 precision (14 narration FPs to recover 2 real claims). The verbs that escape `REPORTING_VERB` and cause the FPs are `planned` / `scheduled` / `presented` / `shared` / `shifted` / `informed` (plan-state datelines, not announcement events).
