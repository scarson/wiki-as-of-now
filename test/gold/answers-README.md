...
| Date | Event | Notes |
|------|-------|-------|
| 2026-06-21 | Pilot batch fetched (Phase 3) | Batch size 9 (see [pilot-batch-rationale.md](pilot-batch-rationale.md)). Auto/escalate split: **1 `agent_auto`** (zumwalt-class_destroyer, grounded on USNI News) / **8 `human_confirmed`** (see [escalation-queue.md](escalation-queue.md)). Heavy escalation is expected and intended (bias-to-escalate; no human in the loop at fetch time). Outcomes: 6 `slipped_still_pending`, 3 `event_occurred`, 0 `unverifiable`. Inter-rater pass: not yet run (Phase 4). First-snapshot hash cross-check (`hashSnapshotBody` === tool `metadata.content_hash_sha256`): **PASS** (zumwalt snapshot, `8698afec…`). |
| 2026-06-21 | Pilot assessed + zumwalt re-grounded | Orchestrator reviewed all 9 pilot records for the relevance the harness can't check. 8 escalations sound. The 1 `agent_auto` (zumwalt) was mis-grounded — the claim is about *testing* CPS in 2025 but the quote was about *fielding* (FY2025→FY2026), an inference. Re-grounded to the snapshot's in-span testing-2027 fact and reclassified `agent_auto`→`human_confirmed`. Gate decision: **PASS, scale to full corpus** with auto-cert disabled for the unattended run + an in-span-resolving-fact preference. |
| 2026-06-21 | Full corpus assembled (32 records) | All 32 `stale: true` gold-set claims now answered (9 pilot + 23 via 3 parallel agents). **All `human_confirmed`** (auto-cert disabled this run; Sam calibrates the auto-tier on review — TSMC/PCI-SIG/Gates MRI/Gateway/Janes-sourced records flagged in the queue as clean Tier-1 candidates). **30 grounded / 2 `unverifiable`** (brightline_west, project_kuiper — no qualifying current-state source). Outcomes: 14 `event_occurred`, 12 `slipped_still_pending`, 1 `event_cancelled`, 3 `superseded`, 2 `unverifiable`. Integrity harness green on all 32; full suite 919 passed. Inter-rater pass (Phase 4): pending. Every record is in [escalation-queue.md](escalation-queue.md) for Sam's spot-check. |
